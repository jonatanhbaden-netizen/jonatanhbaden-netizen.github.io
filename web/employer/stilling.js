...
import { esc, tall, siden, STATUS_TEKST } from '/shared/format.js';
    ...
    <div class="rad-kandidat">
      ...
      <div class="rad-grunner stabel-2">
        ...
        <div class="handlinger">
          ...
          ${s.status !== 'intervju' && s.status !== 'tilbud' ? `<button class="knapp knapp-liten" data-intervju="${s.application_id}" data-navn="${esc(navn)}">Book intervju</button>` : ''}
...
function bindHandlinger() {
  ...
  document.querySelectorAll('[data-intervju]').forEach((k) => {
    k.addEventListener('click', () => {
      ...
      const om = new Date(Date.now() + 3 * 86400000);
      om.setMinutes(0, 0, 0);
      document.getElementById('intervju-tid').value = om.toISOString().slice(0, 16);
      dialog.showModal();
    });
  });
...
dialog.addEventListener('close', async () => {
  ...
  const tid = document.getElementById('intervju-tid').value;
  if (!tid) return;

  feil.textContent = '';
  const { error: iFeil } = await supabase.from('interviews')
    .insert({ application_id: aktivSoknad, scheduled_at: new Date(tid).toISOString(), notes: 'Førstegangsintervju' });
